...
[ IsQuiverAlgebra ],
function( A )
  local Q, num_vert, S, ext2matrix, temp;
  ...
  if not ForAll( [ 1..num_vert ], i -> AdjacencyMatrixOfQuiver( Q )[ i ][ i ] = 0 ) then
    Error( "The quiver of the algebra has at least one loop.\n" );
  fi;
  # 
  # Finding where elements in a minimal set of relations start and end. 
  # 
  S := SimpleRepresentations( A );
  ext2matrix := List( [ 1..num_vert], i -> List( [ 1..num_vert ], j -> Dimension( Ext( 2, S[ i ], S[ j ] ) ) ) ); 
  if not ForAll( [ 1..num_vert ], i -> ext2matrix[ i ][ i ] = 0 ) then
...
[ IsUnitForm ],
function( B )
  local q, A, n, simples, C, flag, v, R, r, j;
...
[ IsUnitForm ],
function( B )
  local q, flag, reductionpairs, m, n, testI, roots, i, temp, j, 
        testII, breakflag, reductionnumber, lastcolumn, lastrow, qq, 
        s;
